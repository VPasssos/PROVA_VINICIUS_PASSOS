...
if($_SERVER["REQUEST_METHOD"] == 'POST'){
    ...
    if($nova_senha !== $confirmar_senha){
        echo "<script>alert('As senha não coencidem');</script>";
    } elseif(strlen($nova_senha <8)){
        echo "<script>alert('Senha deve ter pelomenos 8 caracteres');</script>";
    } elseif($nova_senha === 'temp123'){
        echo "<script>alert('Escolha uma senha diferente');</script>";
    } else {
        $senha_hash = password_hash($nova_senha, PASSWORD_DEFAULT);
        
        // ATUALIZA A SENHA E REMOVE O ESTADOS DE TEMPORARIA
        ...
        if($stmt->execute()){
            session_destroy(); // FINALIZA A SESAO
            echo "<script>alert('Senha Auterada com Suceso! Realize o login novamente');window.location.href='login.php'</script>";
        } else{
            echo "<script>alert('ERRO ao auterar senha!');</script>";
        }
